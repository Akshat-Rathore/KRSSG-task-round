...
SkillType NaoBehavior::selectSkill() {
    ...
    // back and forth
    return kickBalltoGoal();
}


SkillType NaoBehavior::kickBalltoGoal()
{
    VecPosition temp;
    if (worldModel->getUNum() == 1) {
            ...
            WorldObject* teammate = worldModel->getWorldObject( WO_TEAMMATE1 );
            temp=teammate->pos;
        }
        temp.setZ(0);
        double distanceToBall = temp.getDistanceTo(VecPosition(HALF_FIELD_X, 0, 0));

    VecPosition temp1;
    if (worldModel->getUNum()!=1)
    {
        temp1=worldModel->getMyPosition();

    }
    temp1.setZ(0);
    double distanceToDefender=temp.getDistanceTo(temp1)
    if(distanceToDefender <= (distanceToBall*t)){
       return kick(KICK_DRIBBLE,VecPosition(HALF_FIELD_X, 0, 0))
        else{
            continue;
        }
...
SkillType NaoBehavior::kick(const int a,const VecPosition &b){
    kickBall(KICK_FORWARD,b);
